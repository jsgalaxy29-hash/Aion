...
public sealed class MistralProvider : ILanguageModelClient
{
    ...
    public MistralProvider(HttpClient httpClient, IOptions<LanguageModelOptions> options, ILogger<MistralProvider> logger)
    {
        _httpClient = httpClient;
        _options = options.Value.Mistral;
        _logger = logger;
    ...
    public async Task<string> CompleteAsync(string systemPrompt, string userPrompt, CancellationToken ct = default)
    {
        ...
        for (var attempt = 0; attempt <= _options.MaxRetries; attempt++)
        {
            try
            {
                ...
                {
                    var message = $"Mistral error {(int)response.StatusCode}: {body}";
            ...
            {
                _logger.LogWarning(ex, "Retrying Mistral request after failure on attempt {Attempt}", attempt + 1);
                await DelayAsync(attempt, ct);
            }
        }

        throw new InvalidOperationException("Unable to complete Mistral request after retries.");
    }
